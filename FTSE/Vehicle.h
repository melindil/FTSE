...
#pragma pack(push,1)
	typedef struct
	{
		...
		float unkbox[6];
		int32_t unk2;
		uint16_t driver_seqnum;
		uint16_t driver;
		uint16_t controller_seqnum;
		uint16_t controller;
		uint16_t gunner_seqnum;
		uint16_t gunner;
		int32_t maxpassengers;
		uint32_t passenger[7];
		float exitx;
		float exity;
		float exitz;
		char attributes[0x339];
		ActorStatus actorstatus;
		uint16_t repairobject_seqnum;
		uint16_t repairobject_id;
		bool repairneeded;
		bool hasweapon;
		uint16_t weaponseqnum;
		uint16_t weapon;
		uint16_t driver_lefthand_seqnum;
		uint16_t driver_lefthand_id;
		uint16_t gunner_lefthand_seqnum;
		uint16_t gunner_lefthand_id;
		uint16_t inventory_seqnum;
		uint16_t inventory;
		bool runspeed;
		bool inreverse;
		int32_t unk3;
